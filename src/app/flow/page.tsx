...
import {
  ...
  Loader2, Settings, Webhook, Database, MessageSquare, Mail, Sparkles,
  Trash2, SlidersHorizontal, Save, Network, RefreshCw, FileJson, Wand2
} from "lucide-react";
...
const LIBRARY: LibraryItem[] = [
  ...
  { type: "action",  title: "Database Insert",    icon: <Database  className="w-4 h-4" />, desc: "Save to LitLabs Ledger", keywords: ["database", "db", "sql", "save", "store", "persist", "ledger"] },
...
export default function FlowPage() {
  const [nodes, setNodes] = useState<PipelineNode[]>([
    { id: "1", type: "trigger", title: "Webhook Listener",  config: defaultConfig("Webhook Listener"),  status: "idle" },
    { id: "2", type: "agent",   title: "Logic Orchestrator", config: defaultConfig("Logic Orchestrator"), status: "idle" },
  ]);
  ...
  const addNode = (item: LibraryItem) => {
    const newNode: PipelineNode = {
      id: Date.now().toString(), type: item.type, title: item.title,
      config: defaultConfig(item.title), status: "idle",
    };
  ...
  const updateConfig = (id: string, field: string, value: string | number | boolean) => {
    setNodes(prev => prev.map(n => n.id === id ? { ...n, config: { ...n.config, [field]: value } } : n));
  };

  const runPipeline = () => {
    if (nodes.length === 0) { log("[WRN] Pipeline is empty."); return; }
    setIsRunning(true);
    ...
    nodes.forEach((node, idx) => {
      setTimeout(() => {
        ...
        if (node.type === "agent") log(`      -> Tokens: ${Math.floor(Math.random() * 800 + 200)} | Model: ${node.config.model}`);
        setTimeout(() => {
          setNodes(prev => prev.map(n => n.id === node.id ? { ...n, status: "completed" } : n));
          log(`[OK]  Node ${idx + 1} completed`);
          if (idx === nodes.length - 1) {
            setTimeout(() => { log("[SYS] Pipeline execution completed successfully."); setIsRunning(false); }, 400);
          }
        }, 600);
      }, idx * 900);
    });
  ...
  const handleAiBuild = async (e: React.FormEvent) => {
    ...
    if (lowered.match(/webhook|http|endpoint|api|post|request/)) detected.push(LIBRARY[0]);
    if (lowered.match(/cron|schedule|hourly|daily|weekly|timer|interval|periodic/)) detected.push(LIBRARY[1]);
    if (lowered.match(/route|logic|analyze|decide|orchestrate|classify/)) detected.push(LIBRARY[2]);
    if (lowered.match(/summarize|extract|transform|generate|reason|llm|gpt|ai/)) detected.push(LIBRARY[3]);
    if (lowered.match(/database|db|sql|save|store|persist|ledger/)) detected.push(LIBRARY[4]);
    if (lowered.match(/discord|notify|alert|channel|slack|message/)) detected.push(LIBRARY[5]);
    if (lowered.match(/email|mail|smtp/)) detected.push(LIBRARY[6]);
    if (detected.length === 0) detected.push(LIBRARY[1], LIBRARY[3], LIBRARY[4]);

    const built: PipelineNode[] = detected.map((item, i) => ({
      id: (Date.now() + i).toString(), type: item.type, title: item.title,
      config: defaultConfig(item.title), status: "idle",
    }));
    ...
    try {
      const res = await fetch("/api/gemini", {
        ...
        body: JSON.stringify({
          ...
          systemPrompt: "You are a pipeline architect. Output ONLY valid JSON arrays. No explanations.",
        }),
      });
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const geminiNodes: PipelineNode[] = parsed.map((n: any, i: number) => ({
            id: (Date.now() + i).toString(),
            type: (["trigger", "agent", "action"].includes(n.type) ? n.type : "agent") as NodeType,
            title: n.title || "AI Node",
            config: { ...defaultConfig(n.title || ""), ...(n.config || {}) },
            status: "idle",
          }));
    ...
    try {
      const res = await fetch("/api/gemini", {
        ...
        body: JSON.stringify({
          ...
          systemPrompt: "You are a prompt engineering specialist. Output only the rewritten prompt. No preamble.",
        }),
      });
      const data = await res.json();
      if (data.response) updateConfig(nodeId, "prompt", data.response.trim());
      log("[OPT] Prompt optimized via Gemini");
    } catch { log("[WRN] Prompt optimization failed"); }
  ...
  if (!isLoaded) {
    return (
      <div className="min-h-screen flex items-center justify-center font-mono bg-[#050108] text-fuchsia-400">
        <div className="text-center"><div className="text-3xl mb-4 animate-pulse">⚡</div><div>Loading Pipeline...</div></div>
      </div>
    );
  }
  if (!isSignedIn) {
  ...
  return (
    <div className="flex h-full bg-[#050108] text-slate-300 font-sans overflow-hidden relative">
      ...
      <aside className="w-56 shrink-0 border-r border-fuchsia-900/30 bg-[#0a0310]/80 backdrop-blur-xl flex flex-col z-10">
        ...
        <div className="px-3 py-2">
          <div className="flex items-center gap-1.5 bg-[#130720] border border-white/10 rounded-lg px-2 py-1.5">
            ...
            {libraryFilter && <button onClick={() => setLibraryFilter("")} className="text-slate-500 hover:text-white"><X className="w-3 h-3" /></button>}
          </div>
        </div>
            ...
            return (
              <div key={cat} className="space-y-1.5">
                <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest pl-1">
                  {cat === "trigger" ? "1. Triggers" : cat === "agent" ? "2. AI Agents" : "3. Actions"}
                </div>
                  ...
                  return (
                    <div key={idx} onClick={() => addNode(item)}
                      ...
                      <div className="flex items-center mb-1">
                        <div className={`w-5 h-5 rounded flex items-center justify-center mr-2 border ${s.bg} ${s.border} ${s.color}`}>{item.icon}</div>
                        <span className="text-[11px] font-bold text-white">{item.title}</span>
                      </div>
                      <p className="text-[9px] text-slate-500 ml-7 leading-tight">{item.desc}</p>
                    </div>
                  );
                })}
              </div>
      ...
      {/* ── CENTER: Canvas ── */}
      <main className="flex-1 relative flex flex-col items-center overflow-y-auto py-6 z-10" style={{ scrollbarWidth: "none" }}>
        {/* Header */}
        <div className="w-full max-w-lg mb-5 flex items-center justify-between px-1">
          <span className="text-xs font-mono text-slate-500">untitled_pipeline.yaml</span>
        ...
        <div className="w-full max-w-lg flex flex-col items-center pb-32">
          {nodes.length === 0 ? (
            <div className="mt-16 text-center flex flex-col items-center">
              ...
              <h2 className="text-lg font-bold text-slate-400 mb-2">Pipeline Empty</h2>
              <p className="text-sm text-slate-500 max-w-sm">Click tools from the library or ask the AI to generate a pipeline.</p>
            </div>
          ) : nodes.map((node, idx) => {
            const s = NODE_META[node.type];
            const isSelected = selectedNodeId === node.id;
            const status = node.status || "idle";
            const statusDot = status === "running" ? "bg-fuchsia-400 animate-pulse" : status === "completed" ? "bg-emerald-400" : status === "error" ? "bg-red-400" : "bg-slate-600";
            return (
              <React.Fragment key={node.id}>
                <div onClick={() => setSelectedNodeId(node.id)}
                  ...
                  {/* Status badge */}
                  <div className={`absolute -top-2 -right-2 text-[8px] font-bold uppercase px-1.5 py-0.5 rounded-full border
                    ${status === "running" ? "bg-fuchsia-900/80 text-fuchsia-300 border-fuchsia-500/40 animate-pulse" :
                      status === "completed" ? "bg-emerald-900/80 text-emerald-300 border-emerald-500/40" :
                      status === "error" ? "bg-red-900/80 text-red-300 border-red-500/40" :
                      "bg-slate-800 text-slate-400 border-slate-600/30"}`}>
                    {status}
                  </div>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center border ${s.bg} ${s.border} ${s.color}`}>
                        {node.type === "trigger" ? <Zap className="w-4 h-4" /> : node.type === "agent" ? <Cpu className="w-4 h-4" /> : <Box className="w-4 h-4" />}
                      </div>
                      <div>
                        <div className={`text-[9px] font-bold uppercase tracking-widest ${s.color} mb-0.5`}>{s.label}</div>
                        <div className="text-sm font-bold text-white">{node.title}</div>
                      </div>
                      <span className={`w-2 h-2 rounded-full ${statusDot} ml-1`} style={{ boxShadow: status === "running" ? "0 0 6px currentColor" : "none" }} />
                    </div>
                  ...
                  {node.type === "agent" && (
                    <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between text-[10px] font-mono text-slate-400">
                      <span className="truncate flex-1 max-w-[200px]"><span className="text-fuchsia-500/50 mr-2">prompt:</span>{String(node.config.prompt) || "No prompt set..."}</span>
                      <span className="bg-black/30 px-2 py-0.5 rounded border border-white/5 ml-3 shrink-0 text-[9px]">{String(node.config.model)}</span>
                    </div>
                  )}
                </div>
                {idx < nodes.length - 1 && (
                  <div className={`h-8 w-px my-1.5 relative transition-all ${isRunning && (nodes[idx].status === "completed" || nodes[idx].status === "running") ? "bg-gradient-to-b from-fuchsia-400 via-emerald-400 to-emerald-400 shadow-[0_0_8px_rgba(217,70,239,0.5)]" : "bg-gradient-to-b from-fuchsia-500/50 to-emerald-500/50"}`}>
          ...
          <div className="bg-[#0a0310]/95 backdrop-blur-2xl border border-fuchsia-500/40 rounded-2xl p-2 shadow-[0_10px_50px_rgba(217,70,239,0.2)] focus-within:border-fuchsia-400 transition-all">
            <form onSubmit={handleAiBuild} className="flex items-center w-full">
              ...
              <button type="submit" disabled={isGenerating || !aiPrompt.trim()}
                ...
                {isGenerating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 text-fuchsia-400" />}
              </button>
            </form>
          </div>
        </div>
      ...
      <aside className="w-80 shrink-0 border-l border-fuchsia-900/30 bg-[#0a0310]/80 backdrop-blur-xl flex flex-col z-10">
          ...
          {selectedNode ? (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[8px] font-bold uppercase px-1.5 py-0.5 rounded-full ml-auto ${selectedNode.status === "running" ? "bg-fuchsia-900/50 text-fuchsia-300" : selectedNode.status === "completed" ? "bg-emerald-900/50 text-emerald-300" : "bg-slate-800 text-slate-400"}`}>
              ...
              {/* Node name */}
              <div>
                <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Node Name</label>
              ...
              {selectedNode.title === "Webhook Listener" && (
                <>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Endpoint Path</label>
                    <input value={String(selectedNode.config.endpoint || "")} onChange={e => updateConfig(selectedNode.id, "endpoint", e.target.value)}
                      placeholder="/api/v1/ingest" className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white focus:border-amber-500/50 outline-none" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Method</label>
                    <div className="flex gap-1.5">
                      {["GET", "POST", "PUT"].map(m => (
                        <button key={m} onClick={() => updateConfig(selectedNode.id, "method", m)}
                  ...
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Headers</label>
                    <textarea value={String(selectedNode.config.headers || "")} onChange={e => updateConfig(selectedNode.id, "headers", e.target.value)}
              ...
              {selectedNode.title === "Scheduled Interval" && (
                <>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Preset</label>
                    <div className="grid grid-cols-3 gap-1.5">
                      {[
                        ...
                        <button key={p.id} onClick={() => { updateConfig(selectedNode.id, "preset", p.id); updateConfig(selectedNode.id, "cron", p.cron); }}
                  ...
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Cron Expression</label>
                    <input value={String(selectedNode.config.cron || "")} onChange={e => { updateConfig(selectedNode.id, "cron", e.target.value); updateConfig(selectedNode.id, "preset", "custom"); }}
                      placeholder="0 * * * *" className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white font-mono focus:border-amber-500/50 outline-none" />
                  </div>
                </>
              )}
              {/* === AGENT CONFIGS === */}
              {(selectedNode.title === "Logic Orchestrator" || selectedNode.title === "Task Champion") && (
                <>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">AI Model</label>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Temperature</label>
                      <span className="text-[10px] text-fuchsia-400 font-mono">{selectedNode.config.temperature}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">System Prompt</label>
                      <button onClick={() => optimizePrompt(selectedNode.id)} disabled={optimizingPrompt}
                        ...
                        {optimizingPrompt ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <Wand2 className="w-2.5 h-2.5" />} Auto-Optimize
                      </button>
                    </div>
              ...
              {selectedNode.title === "Database Insert" && (
                <>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Target Table</label>
                    <input value={String(selectedNode.config.table || "")} onChange={e => updateConfig(selectedNode.id, "table", e.target.value)}
                      placeholder="pipeline_output" className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white focus:border-emerald-500/50 outline-none" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Ledger Cluster</label>
                    <select value={String(selectedNode.config.cluster || "primary")} onChange={e => updateConfig(selectedNode.id, "cluster", e.target.value)}
                      className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white outline-none cursor-pointer">
              ...
              {selectedNode.title === "Discord Webhook" && (
                <>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Webhook URL</label>
                    <input value={String(selectedNode.config.webhook_url || "")} onChange={e => updateConfig(selectedNode.id, "webhook_url", e.target.value)}
                      placeholder="https://discord.com/api/webhooks/..." className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white focus:border-emerald-500/50 outline-none" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Message Template</label>
                    <textarea value={String(selectedNode.config.message_template || "")} onChange={e => updateConfig(selectedNode.id, "message_template", e.target.value)}
                      rows={3} placeholder="Pipeline completed: {{status}}"
              ...
              {selectedNode.title === "Email Dispatch" && (
                <>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">To</label>
                    <input value={String(selectedNode.config.to || "")} onChange={e => updateConfig(selectedNode.id, "to", e.target.value)}
                      placeholder="user@example.com" className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white focus:border-emerald-500/50 outline-none" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Subject</label>
                    <input value={String(selectedNode.config.subject || "")} onChange={e => updateConfig(selectedNode.id, "subject", e.target.value)}
                      placeholder="Pipeline Alert" className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white focus:border-emerald-500/50 outline-none" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Body</label>
                    <textarea value={String(selectedNode.config.body || "")} onChange={e => updateConfig(selectedNode.id, "body", e.target.value)}
                      rows={3} placeholder="Pipeline finished execution."
                      className="w-full bg-[#130720] border border-white/10 rounded-lg p-2 text-xs text-white focus:border-emerald-500/50 outline-none resize-none" />
                  </div>
                </>
              )}
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-6 opacity-40">
              ...
              <h3 className="text-xs font-bold text-white mb-1">No Node Selected</h3>
              <p className="text-[10px] text-slate-400">Click a block in the canvas to configure it.</p>
            </div>
          )}
        </div>
        ...
        <div className="h-[220px] bg-[#050108] border-t border-fuchsia-900/40 flex flex-col relative">
          <div className="bg-[#130720]/80 px-3 py-1.5 border-b border-fuchsia-900/30 flex items-center justify-between absolute top-0 w-full z-10 backdrop-blur-md">
            <div className="flex items-center gap-2"><Terminal className="w-3 h-3 text-slate-400" /><span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Execution Log</span></div>
            <div className="flex items-center gap-2">
              <button onClick={clearTerminal} className="text-[8px] text-slate-500 hover:text-slate-300 transition-colors">Clear</button>
              <div className={`w-1.5 h-1.5 rounded-full ${isRunning ? "bg-fuchsia-500 animate-pulse shadow-[0_0_8px_rgba(217,70,239,0.8)]" : "bg-slate-700"}`} />
            </div>
          </div>
          <div className="flex-1 p-3 pt-9 overflow-y-auto font-mono text-[9px] space-y-1 flex flex-col" style={{ scrollbarWidth: "none" }}>
            {terminalLogs.map((log, i) => {
              ...
              return <div key={i} className={`${cls} leading-snug`}>{log}</div>;
            })}
            <div ref={bottomRef} className="text-fuchsia-500 animate-pulse font-bold">_</div>
          </div>
        </div>
      </aside>
      {/* ── YAML Export Modal ── */}
      {showYaml && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.85)" }} onClick={() => setShowYaml(false)}>
          <div onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-xl border overflow-hidden flex flex-col"
            style={{ backgroundColor: "#0a0310", borderColor: "rgba(217,70,239,0.3)", maxHeight: "80vh" }}>
            <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-white">Pipeline YAML</span>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={copyYaml} className="flex items-center gap-1 text-[10px] px-2 py-1 rounded border transition-all hover:bg-white/5"
                  style={{ borderColor: "rgba(255,255,255,0.1)", color: yamlCopied ? "#22c55e" : "#d946ef" }}>
                  {yamlCopied ? <><Check className="w-3 h-3" /> Copied</> : <><Copy className="w-3 h-3" /> Copy</>}
                </button>
                <button onClick={() => setShowYaml(false)} className="text-slate-500 hover:text-white transition-colors"><X className="w-4 h-4" /></button>
              </div>
            </div>
            <div className="p-4 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
              <pre className="font-mono text-[10px] leading-relaxed whitespace-pre-wrap" style={{ color: "#94a3b8" }}>{yaml}</pre>
            </div>
          </div>
        </div>
      )}
