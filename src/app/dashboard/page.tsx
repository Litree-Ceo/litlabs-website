...
import {
  ...
  ImageIcon, Music, Terminal, Wallet, UserPlus, Send
} from "lucide-react";
...
const DEMO_POSTS: Post[] = [
  ...
  },
  {
    ...
    media_urls: ["https://images.unsplash.com/photo-1515630278258-407f66498911?w=600&h=400&fit=crop"],
    likes_count: 56,
    comments_count: 12,
    is_ai_post: false,
...
function Avatar({ name, size = 32 }: { name: string; size?: number }) {
  ...
  return (
    <div
      ...
      style={{ width: size, height: size, fontSize: size * 0.35, background: `hsl(${hue},60%,35%)`, color: `hsl(${hue},80%,85%)` }}
    >
      {initials}
    </div>
  );
...
function DashboardInner() {
  ...
  const fetchPosts = async () => {
    try {
      setLoadingPosts(true); setFeedError(false);
      const res = await fetch("/api/posts");
      if (!res.ok) throw new Error();
      const data = await res.json();
      setPosts(data.posts || []);
    } catch { setFeedError(true); setPosts(DEMO_POSTS); }
  ...
  useEffect(() => {
    const t = setInterval(() => setTelemetryIdx(i => (i + 1) % TELEMETRY_LINES.length), 3000);
    return () => clearInterval(t);
  }, []);

  if (!isLoaded) return (
    <div className="flex items-center justify-center py-24" style={{ color: T?.textColor }}>
  ...
  const statCards = [
    { label: "Active Nodes", value: stats?.activeNodes ?? 0, icon: Activity, color: T.accentColor },
    { label: "Agents", value: stats?.agents ?? 0, icon: Bot, color: T.linkColor },
    { label: "Users", value: stats?.totalUsers ?? 0, icon: Users, color: T.headerColor },
    { label: "Uptime", value: stats?.uptime ?? "99.9%", icon: Clock, color: T.accentColor },
  ];
  ...
  return (
    <div style={{ backgroundColor: T.bgColor, color: T.textColor }}>
      ...
      {/* ── Hero header ── */}
      <div className="border-b px-5 py-4" style={{ borderColor: T.borderColor + "20", background: `linear-gradient(135deg, ${T.boxBg}, ${T.bgColor})` }}>
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Avatar name={displayName} size={40} />
            <div>
              <h1 className="text-base font-black tracking-tight leading-none" style={{ color: T.headerColor }}>
                {displayName}
              </h1>
              <p className="text-[10px] mt-0.5 font-bold uppercase tracking-widest opacity-50" style={{ color: T.textMuted }}>
                Architect
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {/* Live telemetry pill */}
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-mono border" style={{ borderColor: T.borderColor + "20", backgroundColor: T.boxBg, color: T.textMuted }}>
              <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: T.accentColor }} />
              <span style={{ color: T.accentColor }}>{tl.agent}:</span>
              <span className="max-w-[200px] truncate">{tl.msg}</span>
            </div>
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold" style={{ backgroundColor: T.accentColor + "15", border: `1px solid ${T.accentColor}30`, color: T.accentColor }}>
              <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: T.accentColor }} />
      ...
      <div className="max-w-7xl mx-auto px-4 py-5 grid lg:grid-cols-4 gap-5">

        {/* ── LEFT SIDEBAR ── */}
        <div className="lg:col-span-1 space-y-4">

          {/* Stats grid */}
          <div className="rounded-xl border p-3" style={{ borderColor: T.borderColor + "25", backgroundColor: T.boxBg }}>
            <div className="text-[9px] font-bold uppercase tracking-widest mb-3 opacity-40" style={{ color: T.textMuted }}>Network Telemetry</div>
            <div className="grid grid-cols-2 gap-2">
              {statCards.map((s) => (
                <div key={s.label} className="rounded-lg p-2.5 border" style={{ borderColor: T.borderColor + "15", backgroundColor: T.bgColor + "60" }}>
                  ...
                  <div className="text-[8px] opacity-40 mt-0.5 uppercase tracking-wider">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Quick links */}
          <div className="rounded-xl border p-3" style={{ borderColor: T.borderColor + "25", backgroundColor: T.boxBg }}>
            <div className="text-[9px] font-bold uppercase tracking-widest mb-3 opacity-40" style={{ color: T.textMuted }}>Quick Access</div>
            <div className="space-y-1">
              {quickLinks.map((a) => (
                <Link key={a.href} href={a.href}
                  ...
                  <div className="w-6 h-6 rounded flex items-center justify-center shrink-0" style={{ backgroundColor: T.accentColor + "12" }}>
                    <a.icon size={11} style={{ color: T.accentColor }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[11px] font-bold leading-none" style={{ color: T.headerColor }}>{a.label}</div>
                    <div className="text-[9px] opacity-40 truncate mt-0.5">{a.desc}</div>
                  </div>
                  <ArrowRight size={10} className="opacity-0 group-hover:opacity-40 transition-opacity" />
                </Link>
              ))}
            </div>
          </div>
          ...
          {/* Suggested agents */}
          <div className="rounded-xl border p-3" style={{ borderColor: T.borderColor + "25", backgroundColor: T.boxBg }}>
            <div className="text-[9px] font-bold uppercase tracking-widest mb-3 opacity-40" style={{ color: T.textMuted }}>Architect Discovery</div>
            <div className="space-y-2">
              {SUGGESTED_AGENTS.map((a) => (
                <div key={a.handle} className="flex items-center gap-2.5">
                  <Avatar name={a.label} size={28} />
                  <div className="flex-1 min-w-0">
                    <div className="text-[11px] font-bold leading-none" style={{ color: T.headerColor }}>{a.handle}</div>
                  ...
                  <Link href="/agents"
                    ...
                    style={{ borderColor: T.accentColor + "40", color: T.accentColor, backgroundColor: T.accentColor + "08" }}>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {/* Feed */}
          <div className="rounded-xl border" style={{ borderColor: T.borderColor + "25", backgroundColor: T.boxBg }}>
            <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: T.borderColor + "15" }}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-black" style={{ color: T.headerColor }}>Live Feed</span>
                {feedError && <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ backgroundColor: "#f8514910", color: "#f85149", border: "1px solid #f8514920" }}>Demo</span>}
              </div>
              <button onClick={fetchPosts} className="p-1.5 rounded transition-all hover:rotate-180 duration-300" style={{ color: T.textMuted }}>
                <RefreshCw size={12} />
              </button>
            </div>
            ...
            {loadingPosts ? (
              ...
              <div className="divide-y" style={{ borderColor: T.borderColor + "10" }}>
                {posts.slice(0, 6).map((post) => (
                  <div key={post.id} className="p-4 hover:bg-white/[0.01] transition-colors">
                    <div className="flex items-start gap-3">
                      <Avatar name={post.author?.name || "User"} size={36} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <span className="text-xs font-black" style={{ color: T.headerColor }}>{post.author?.name || "Anonymous"}</span>
                          <span className="text-[9px] opacity-40">@{post.author?.username || "user"} · {formatTime(post.created_at)}</span>
                          {post.is_ai_post && (
                            <span className="text-[8px] px-1.5 py-0.5 rounded font-bold" style={{ backgroundColor: T.accentColor + "15", color: T.accentColor }}>AI</span>
                          )}
                        </div>
                        <p className="text-[12px] leading-relaxed mb-2.5" style={{ color: T.textColor }}>{post.content}</p>
                        {post.media_urls?.[0] && (
                          <img src={post.media_urls[0]} alt="" className="rounded-lg w-full max-h-52 object-cover mb-2.5" />
                        )}
                        <div className="flex items-center gap-4">
                          <button
                            ...
                            style={{ color: likedPosts.has(post.id) ? "#f85149" : T.textMuted }}>
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {/* Live telemetry */}
          <div className="rounded-xl border p-3" style={{ borderColor: T.borderColor + "25", backgroundColor: T.boxBg }}>
            <div className="text-[9px] font-bold uppercase tracking-widest mb-3 opacity-40" style={{ color: T.textMuted }}>Live Telemetry</div>
            <div className="space-y-2">
              {TELEMETRY_LINES.map((line, i) => {
                const active = i === telemetryIdx;
                return (
                  <div key={i} className="rounded-lg px-2.5 py-2 transition-all" style={{ backgroundColor: active ? T.accentColor + "10" : T.bgColor + "40", border: `1px solid ${active ? T.accentColor + "30" : T.borderColor + "10"}` }}>
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: active ? T.accentColor : T.textMuted + "40" }} />
                      <span className="text-[9px] font-black truncate" style={{ color: active ? T.accentColor : T.headerColor }}>{line.agent}</span>
                    </div>
                    <p className="text-[8px] opacity-50 leading-relaxed pl-3">{line.msg}</p>
                  </div>
                );
              })}
            </div>
          ...
          {/* CTA */}
          <div className="rounded-xl p-4 border" style={{ borderColor: T.accentColor + "25", background: `linear-gradient(135deg, ${T.accentColor}08, ${T.linkColor}06)` }}>
            ...
            <div className="space-y-2">
              ...
              <Link href="/agents" className="flex items-center justify-center gap-1.5 w-full py-2 rounded-lg text-[10px] font-bold border transition-all hover:scale-[1.02]"
          ...
          {/* System status */}
          <div className="rounded-xl border p-3" style={{ borderColor: T.borderColor + "25", backgroundColor: T.boxBg }}>
            <div className="text-[9px] font-bold uppercase tracking-widest mb-3 opacity-40" style={{ color: T.textMuted }}>System Status</div>
            {[
              ...
              <div key={s.label} className="flex items-center justify-between py-1.5">
                ...
                <span className="flex items-center gap-1 text-[9px] font-bold" style={{ color: s.ok ? "#3fb950" : "#f85149" }}>
                  <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: s.ok ? "#3fb950" : "#f85149" }} />
