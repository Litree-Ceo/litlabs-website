...
const SAMPLE_TRACKS = [
  { url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3", title: "Synthwave Dreams" },
  { url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3", title: "Neon Nights" },
  { url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3", title: "Cyber Pulse" },
  { url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-4.mp3", title: "Digital Horizon" },
  { url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-8.mp3", title: "Electric Sky" },
  { url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-10.mp3", title: "Future City" },
];
...
export default function AudioTool() {
  ...
  const handleDownload = (url: string, label: string) => {
    if (url.startsWith("data:")) {
      setError("Browser TTS audio can't be downloaded. Use the play button to listen.");
      return;
    }
      ...
      <div className="grid lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="border rounded-lg p-3" style={{ borderColor: T.borderColor, backgroundColor: T.boxBg }}>
            ...
            <textarea value={text} onChange={e => { setText(e.target.value); setError(null); }}
              placeholder={mode === "tts" ? "Hello world, this is a test of text-to-speech..." : "EDM trap beat with heavy bass drops..."}
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {VOICES.map(v => (
                  <button key={v.id} onClick={() => setVoice(v.id)} disabled={isGenerating}
                    ...
                    style={{ borderColor: voice === v.id ? T.accentColor : T.borderColor, color: T.textColor }}>
              ...
              <div className="space-y-1.5">
                {MUSIC_MODELS.map(m => (
                  <button key={m.id} onClick={() => setMusicModel(m.id)} disabled={isGenerating}
                    ...
                    style={{ borderColor: musicModel === m.id ? T.accentColor : T.borderColor, backgroundColor: musicModel === m.id ? T.accentColor + "10" : "transparent", color: T.textColor }}>
                    <div className="flex items-center gap-2">
                      <span className="font-bold">{m.label}</span>
                      <span className="opacity-60">{m.desc}</span>
                    </div>
                    <span className="px-1.5 py-0.5 rounded border text-[9px]" style={{ borderColor: T.borderColor }}>{m.cost} 🪙</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {error && (
            <div className="flex items-center gap-2 p-2 rounded border text-[11px]" style={{ borderColor: "#ef4444", color: "#ef4444", backgroundColor: "rgba(239,68,68,0.1)" }}>
              <AlertTriangle size={12} /> {error}
            </div>
          )}

          <button onClick={handleGenerate} disabled={!text.trim() || text.trim().length < 3 || !canAfford || isGenerating}
          ...
          {mode === "tts" && (
            <div className="text-[10px] p-2 rounded border" style={{ borderColor: T.borderColor, color: T.textMuted }}>
              💡 Browser TTS plays directly. Audio file downloads coming soon with backend generation.
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {current && (
            <div className="border rounded-lg p-4" style={{ borderColor: T.borderColor, backgroundColor: T.boxBg }}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] uppercase tracking-widest" style={{ color: T.textMuted }}>
                  {current.status === "generating" ? "Generating..." : current.status === "succeeded" ? "Ready" : "Failed"}
                </span>
                <span className="text-[10px] opacity-60">{current.mode === "tts" ? "TTS" : "Music"}</span>
              </div>

              {current.status === "generating" ? (
                <div className="flex items-center gap-3 py-6">
                  <Loader2 size={18} className="animate-spin" style={{ color: T.accentColor }} />
                  <span className="text-sm" style={{ color: T.textMuted }}>Creating audio...</span>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <button onClick={() => togglePlay(current.id, current.audioUrl!)}
                      ...
                      {playingId === current.id ? <><Pause size={12} /> Pause</> : <><Play size={12} /> Play</>}
                    </button>
                  ...
                  {current.mode === "music" && (
                    <div className="text-xs" style={{ color: T.textMuted }}>
                      🎵 Sample track generated based on your prompt. Full AI music generation coming soon.
                    </div>
                  )}
                </div>
              ) : current.status === "failed" ? (
                <div className="text-sm" style={{ color: "#ef4444" }}>{current.error || "Failed"}</div>
              ) : null}
            </div>
          )}
          ...
          {history.length > 0 && (
            <div className="border rounded-lg p-3" style={{ borderColor: T.borderColor, backgroundColor: T.boxBg }}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] uppercase tracking-widest flex items-center gap-1.5" style={{ color: T.textMuted }}>
                  <History size={12} /> Recent ({history.length})
                </span>
                <button onClick={handleClear} className="text-[10px] hover:opacity-70" style={{ color: T.textMuted }}>Clear</button>
              </div>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {history.map(g => (
                  <div key={g.id} className="flex items-center justify-between p-2 rounded border" style={{ borderColor: T.borderColor }}>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[10px] opacity-50 flex items-center gap-2">
                        ...
                        <span>•</span>
                        <span>{g.cost} 🪙</span>
                        {g.status === "failed" && <span style={{ color: "#ef4444" }}>Failed</span>}
                      </div>
                    </div>
                    {g.audioUrl && g.status === "succeeded" && (
                      <div className="flex items-center gap-1 ml-2">
                        <button onClick={() => togglePlay(g.id, g.audioUrl!)} className="p-1.5 rounded hover:opacity-80" style={{ color: T.accentColor }}>
                          {playingId === g.id ? <Pause size={14} /> : <Play size={14} />}
                        </button>
                        <button onClick={() => handleDownload(g.audioUrl!, g.mode)} className="p-1.5 rounded hover:opacity-80" style={{ color: T.textMuted }}>
                          <Download size={14} />
                        </button>
                      </div>
                    )}
          ...
          {!current && history.length === 0 && (
            <div className="border rounded-lg p-6 text-center" style={{ borderColor: T.borderColor, backgroundColor: T.boxBg }}>
              <Music size={24} className="mx-auto mb-2 opacity-30" />
              <p className="text-sm" style={{ color: T.textMuted }}>Your generated audio will appear here</p>
            </div>
          )}
        </div>
      </div>
