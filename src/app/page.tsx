...
function CRTOverlay({ enabled }: { enabled: boolean }) {
  ...
  return (
    <div
      className="fixed inset-0 pointer-events-none z-[9999] opacity-[0.08] mix-blend-overlay"
      style={{
        background:
...
export default function HomePage() {
  ...
  const openChat = (
    ...
    if (existing) {
      setChats(
        chats.map((c) =>
          c.id === existing.id ? { ...c, minimized: false } : c,
        ),
      );
      return;
    }
  ...
  const minimizeChat = (id: string) =>
    setChats(
      chats.map((c) => (c.id === id ? { ...c, minimized: !c.minimized } : c)),
    );
  ...
  const sendChat = async (id: string, msg: string) => {
    ...
    try {
      const res = await fetch("/api/gemini", {
        ...
        body: JSON.stringify({ message: msg, agentId: chat.agentId }),
      });
  ...
  if (!isLoaded) {
    ...
    return (
      <div className="min-h-screen" style={{ backgroundColor: C.bgColor }}>
        <div className="max-w-7xl xl:max-w-[1600px] mx-auto px-3 sm:px-4 pt-4 pb-12">
          <div
            className="mb-4 p-3 h-12 rounded animate-pulse"
            style={{ backgroundColor: C.boxBg }}
          />
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                style={{
                  ...
                  color: crtEnabled ? C.linkColor : C.textMuted,
                }}
              >
                {crtEnabled ? "✓ CRT" : "CRT"}
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[200px_1fr_240px] lg:grid-cols-[260px_1fr_300px] xl:grid-cols-[280px_1fr_320px] gap-4">
            ...
            <aside className="space-y-4 min-w-0">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-[9px] opacity-60">ONLINE</span>
                    </div>
                  </div>
                </div>
                <div className="mb-3">
                  <div className="text-[9px] uppercase opacity-40 mb-1">
                    Mood
                  </div>
                  <select
                    value={profile.mood}
                    onChange={(e) => updateProfile({ mood: e.target.value })}
                    className="w-full p-1.5 text-xs border bg-transparent"
              ...
              >
                ...
                {[
                  ...
                  <Link
                    key={link.label}
                    href={link.href}
                    className="flex items-center gap-2 p-2 border hover:opacity-80 mb-1"
                    style={{ borderColor: C.borderColor }}
                  >
              ...
              >
                ...
                <button
                  ...
                  className="w-full py-2 text-xs font-bold border disabled:opacity-30"
                  style={{
                    borderColor: dailyClaimed ? C.borderColor : C.linkColor,
                    color: dailyClaimed ? C.textMuted : C.linkColor,
                  }}
                >
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <Terminal size={14} style={{ color: C.headerColor }} />
                  <span
                ...
                <button
                  className="w-full py-2 text-xs font-bold border hover:opacity-80"
            ...
            <section className="space-y-4">
              ...
              >
                <textarea
                  ...
                  placeholder="What's on your mind? Mention code, marketing, data..."
                ...
                <div className="flex justify-between items-center mt-2">
                  <span className="text-[9px] opacity-40">
                    Keywords trigger AI replies
                  </span>
                  <button
                    onClick={handlePost}
                    disabled={!newPost.trim()}
                    className="px-4 py-1.5 text-xs font-bold border disabled:opacity-30"
                    style={{ borderColor: C.linkColor, color: C.linkColor }}
                  >
                    Post
                  </button>
                </div>
                        ...
                        >
                          <div className="flex items-center gap-1 mb-1">
                            <span className="text-xs">
                              {
                                TOP_AGENTS.find((a) => a.id === reply.agentId)
                                  ?.icon
                              }
                            </span>
                            <span
            ...
            <aside className="space-y-4 min-w-0">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {TOP_AGENTS.map((agent) => (
                    <button
                      key={agent.id}
                      onClick={() =>
                        openChat(agent.id, agent.name, agent.icon, agent.color)
                      }
                      className="text-center group"
                    >
                      <div
                        className="w-full aspect-square border-2 flex flex-col items-center justify-center mb-1 transition-all group-hover:scale-105"
                        style={{
                      ...
                      >
                        ...
                        <span
                          className={`w-2 h-2 rounded-full mt-1 ${agent.status === "online" ? "bg-green-500" : agent.status === "busy" ? "bg-yellow-500" : "bg-gray-500"}`}
                        />
                      </div>
              ...
              >
                ...
                {[
                  { name: "Alex Chen", handle: "@alexchen", icon: "👨‍💻" },
                  { name: "Sarah K.", handle: "@sarahk", icon: "👩‍💼" },
                  { name: "Mike Dev", handle: "@mikedev", icon: "🧙‍♂️" },
                ].map((builder) => (
                  <div
                    key={builder.handle}
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{ borderColor: C.borderColor, color: C.linkColor }}
                    >
                      Add
                    </button>
                  </div>
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold">System Telemetry</span>
                </div>
              ...
              >
                ...
                <button
                  ...
                  className="mt-2 text-[10px] px-3 py-1 border hover:opacity-80"
                  style={{ borderColor: C.borderColor }}
                >
                  +1 Visit
                </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_320px] gap-4">
            ...
            <aside className="space-y-4">
              ...
              >
                ...
                {TOP_AGENTS.slice(0, 4).map((agent) => (
                  <Link
                    ...
                    className="flex items-center gap-2 p-2 border mb-2 hover:opacity-80"
                    style={{ borderColor: C.borderColor }}
                  >
              ...
              <div
                ...
                style={{ backgroundColor: C.boxBg, borderColor: C.borderColor }}
              >
                <div className="text-xs font-bold mb-2 uppercase opacity-50">
                  📊 Live Stats
                </div>
            ...
            <section className="space-y-4">
              ...
              <div
                className="border-2 p-4 opacity-50"
                style={{ backgroundColor: C.boxBg, borderColor: C.borderColor }}
              >
              ...
              {[
                {
                  ...
                  avatar: "🎯",
                  content:
                    "🚀 The Boardroom is now LIVE! Multi-agent orchestration has never been this smooth. Who's ready to deploy their AI workforce?",
                  time: "2h ago",
                  likes: 147,
                  comments: 23,
                  agentReplies: [
                ...
                },
                {
                  ...
                  avatar: "👨‍💻",
                  content:
                    "Just built my first Code Champion agent and it wrote an entire React component for me. Mind = blown 🤯 #AI #Coding",
                  time: "4h ago",
                  likes: 89,
                  comments: 12,
                },
                {
                  ...
                  avatar: "👩‍💼",
                  content:
                    "My Social Dominator agent just planned my entire content calendar for the month. 30 posts scheduled in 5 minutes. This is the future.",
                  time: "6h ago",
                  likes: 234,
                  comments: 45,
                  agentReplies: [
                ...
                },
                {
                  ...
                  avatar: "💻",
                  content:
                    "New update: TypeScript generics support is now live. Build type-safe agents with full autocomplete. Check the docs! 📝",
                  time: "8h ago",
                  likes: 67,
                  comments: 8,
                },
                {
                  ...
                  avatar: "🧙‍♂️",
                  content:
                    "Generated 50 AI images this morning using the Studio. The quality is insane. Here are my favorites 👇",
                  time: "12h ago",
                  likes: 156,
                  comments: 34,
                },
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    <Link
                      href={`/profile/${post.author.toLowerCase().replace(/\s+/g, "")}`}
                      className="w-10 h-10 border flex items-center justify-center text-lg shrink-0 hover:opacity-80"
                      style={{ borderColor: C.borderColor }}
                    >
                      {post.avatar}
                    </Link>
            ...
            <aside className="space-y-4">
              ...
              >
                ...
                <p className="text-[10px] opacity-60 mb-3">
                  Build AI agents, earn LitCoins, connect with creators.
                </p>
                <Link
                  href="/sign-up"
              ...
              >
                ...
                {[
                  ...
                  <Link
                    key={builder.handle}
                    href={`/profile/${builder.handle.replace("@", "")}`}
                    className="flex items-center gap-2 p-2 border mb-1 hover:opacity-80"
                    style={{ borderColor: C.borderColor }}
                  >
                    <span className="text-lg">{builder.icon}</span>
                    <div className="flex-1 min-w-0">
              ...
              >
                ...
                <ul className="space-y-1 text-[10px] opacity-70">
                  <li className="flex items-center gap-1">
                    <Sparkles size={10} style={{ color: C.headerColor }} />{" "}
                    Build unlimited AI agents
                  </li>
                  <li className="flex items-center gap-1">
                    <Coins size={10} style={{ color: C.linkColor }} /> Earn
                    LitCoins for activity
                  </li>
                  <li className="flex items-center gap-1">
                    <Users size={10} style={{ color: C.success }} /> Connect
                    with 50K+ builders
                  </li>
                  <li className="flex items-center gap-1">
                    <Zap size={10} style={{ color: C.warning }} /> Automate your
                    workflow
                  </li>
                </ul>
              </div>
      ...
      {chats.length > 0 && (
        <div className="fixed bottom-0 right-0 z-50 flex items-end gap-2 p-4">
          {chats.map((chat, idx) => (
            <div
              key={chat.id}
              className="border-2 flex flex-col shadow-2xl"
              style={{
                width: "280px",
                height: chat.minimized ? "40px" : "350px",
                backgroundColor: C.boxBg,
                borderColor: chat.color,
                order: chats.length - idx,
              }}
            >
              <div
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-lg">{chat.agentIcon}</span>
                  <span
                    className="text-xs font-bold"
                ...
                <>
                    ...
                    {chat.messages.map((m: ChatMessage, i: number) => (
                      ...
                      >
                        <div
                          ...
                          style={{
                            backgroundColor:
                              m.role === "user" ? chat.color + "30" : C.bgColor,
                            borderColor:
                              m.role === "user" ? chat.color : C.borderColor,
                            color: C.textColor,
                          }}
                        >
                          {m.content}
                  ...
                  >
                    <input
                      type="text"
                      placeholder="Message..."
                      className="flex-1 px-2 py-1 text-xs bg-transparent border outline-none"
                      style={{ borderColor: C.borderColor, color: C.textColor }}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          sendChat(
